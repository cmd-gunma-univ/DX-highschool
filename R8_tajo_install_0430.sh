...
JUPYTER_PORT="8888"
NTP_SERVERS_DEFAULT="192.168.0.2 ntp.jst.mfeed.ad.jp ntp.nict.jp time.google.com"
...
# nginx から 127.0.0.1:${JUPYTER_PORT} へ転送する構成
...
[Unit]
Description=Jupyter Lab on ${VENV_DIR}
...
ExecStart=${VENV_DIR}/bin/python -m jupyterlab --config=${HOME}/.jupyter/jupyter_lab_config.py
Restart=always
RestartSec=5
...
cat > /tmp/10-dx-school-ntp.conf <<EOL
[Time]
NTP=${NTP_SERVERS_DEFAULT}
FallbackNTP=ntp.jst.mfeed.ad.jp ntp.nict.jp time.google.com
EOL
sudo mv /tmp/10-dx-school-ntp.conf /etc/systemd/timesyncd.conf.d/10-dx-school-ntp.conf
...
: "${IPADDR:?IPADDR is required}"
: "${PREFIX:?PREFIX is required}"
: "${GATEWAY:?GATEWAY is required}"
: "${DNS:?DNS is required}"
: "${WIFI_SSID:?WIFI_SSID is required}"
: "${WIFI_PASS:?WIFI_PASS is required}"

NTP_SERVERS="${NTP_SERVERS:-192.168.0.2 ntp.jst.mfeed.ad.jp ntp.nict.jp time.google.com}"
...
# 127.0.0.1 は localhost、127.0.1.1 は Debian/Raspberry Pi 系で自ホスト名用に使う。
...
echo "=== 証明書登録 ==="
CERT_UPDATED="no"
for CERT_NAME in default_ca_sha2.crt sharedpx_ca_sha2.crt; do
  if [ -f "${BOOT}/${CERT_NAME}" ]; then
    cp "${BOOT}/${CERT_NAME}" "/usr/local/share/ca-certificates/${CERT_NAME}"
    CERT_UPDATED="yes"
  fi
done
if [ "$CERT_UPDATED" = "yes" ]; then
  update-ca-certificates || true
fi

echo "=== Wi-Fi / IP / DNS 設定 ==="
CONN="wlan0"
...
if [ "${USE_PROXY:-no}" = "yes" ]; then
  : "${PROXY_URL:?PROXY_URL is required when USE_PROXY=yes}"
  cat > /etc/environment <<EOPROXY
http_proxy=${PROXY_URL}
https_proxy=${PROXY_URL}
HTTP_PROXY=${PROXY_URL}
HTTPS_PROXY=${PROXY_URL}
no_proxy=localhost,127.0.0.1,127.0.1.1,${HOSTNAME},${HOSTNAME}.local,${IPADDR}
NO_PROXY=localhost,127.0.0.1,127.0.1.1,${HOSTNAME},${HOSTNAME}.local,${IPADDR}
EOPROXY
...
cat > /etc/systemd/timesyncd.conf.d/10-dx-school-ntp.conf <<EONTP
[Time]
NTP=${NTP_SERVERS}
FallbackNTP=ntp.jst.mfeed.ad.jp ntp.nict.jp time.google.com
EONTP
...
# DNSが通る場合のみ待つ。プロキシ環境では一般NTP名が引けない場合があるため、失敗しても続行。
for i in $(seq 1 20); do
  if getent hosts ntp.jst.mfeed.ad.jp >/dev/null 2>&1 || getent hosts ntp.nict.jp >/dev/null 2>&1 || getent hosts time.google.com >/dev/null 2>&1; then
    break
  fi
...
echo "=== Jupyter 接続確認 ==="
sleep 3
curl -I http://127.0.0.1:8888 2>/dev/null | head -n 1 || true
curl -I http://127.0.0.1/ 2>/dev/null | head -n 1 || true

echo "=== ゴールデンSD化のための注意 ==="
echo "このSDを複製したあと、各SDの boot パーティションに device.conf を置いてください。"
echo "プロキシありの場合は default_ca_sha2.crt と sharedpx_ca_sha2.crt も boot パーティションに置いてください。"
echo "32GB SD用イメージは、32GB SDでゴールデンSDを作るか、Linux上で PiShrink を使って縮小してください。"
